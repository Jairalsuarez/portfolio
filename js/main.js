...
habilidades.forEach((skill, index) => {
    ...
    card.className = "animate-on-scroll opacity-0 translate-y-10 group bg-white flex flex-col p-8 gap-4 justify-center items-center border border-slate-100 rounded-3xl shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 cursor-default";
    card.style.transitionDelay = `${index * 100}ms`;
    
    card.innerHTML = `
        <div class="bg-slate-50 p-6 rounded-2xl group-hover:bg-blue-50 group-hover:rotate-6 transition-all duration-500">
            <img class="w-16 h-16 object-contain" src="${skill.image}" alt="${skill.nombre}">
        </div>
        <div class="text-center">
            <h3 class="font-black text-xl text-blue-950">${skill.nombre}</h3>
            <span class="text-blue-500 text-sm font-bold uppercase tracking-widest">${skill.experiencia}</span>
        </div>
    `;
